...
class TranslationPromptBuilderTest {
    ...
    fun `prompt contains stable ids ordered glossary and explicit sound-effect policy`() {
        ...
        assertTrue(messages.system.contains("response items count equals the request items count"))
        assertTrue(messages.system.contains("translation must be a non-empty JSON string"))
        assertTrue(messages.system.contains("copy its source exactly"))
