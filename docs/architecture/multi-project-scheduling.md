...
- The local lane has capacity one. Detection, OCR, cleanup, typesetting, and export all acquire
...
cleanup and typesetting, then directly to export. There is no runtime quality-audit or automatic
quality-repair stage between `TYPESETTING` and `EXPORT`.
...
therefore reconstructs the next stage rather than restarting a whole chapter. Failed or cancelled
projects are paused; tapping continue reactivates only that project.
