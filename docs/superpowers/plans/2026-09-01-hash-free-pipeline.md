...
**Goal:** Remove first-party content-derived identity/integrity computations and their domain fields while preserving existing projects, models, output, recovery, and reading state.

**Architecture:** All persisted identities become opaque safe IDs. New roots use UUIDs and children use persisted structural IDs. Freshness and recovery compare explicit schemas, policies, model descriptors, ordered page lineage, and upstream IDs. File handling trusts atomic writes, byte lengths, decodability, tensor signatures, and runtime capabilities without full-content verification.
...
- Do not introduce another content-derived identifier or integrity value.
- Preserve old 64-character IDs and legacy model directory names as opaque names; do not rewrite large files.
...
- Add legacy-manifest coverage proving a removed unknown source-integrity field is ignored and the old stored path/page ID remains usable.
- Remove source-integrity fields and import-time content scans; assign new page IDs from `IdSource` while streaming the copy once.
- Replace content-derived shelf identifiers with ordinary project/source metadata and accept older shelf JSON without rewriting archived images.
...
- Add tests proving a valid same-length model is accepted through structure/capability checks and that an existing legacy-named directory is found and reused.
- Remove content-derived fields, matching error codes, and full-file scans from detector, OCR, text-segmenter, and neural model packages.
...
- Remove build/tool content-scan commands and constants while retaining asset existence/length checks and model construction checks.
...
- Remove source/output/glossary integrity fields from current contracts; JSON decoders ignore legacy extras.
...
- Remove cleanup/typesetting/export content-derived image values and SAF content readback; retain byte length, atomic publication, and exact generation-name checks.
...
- Search first-party production/build/tool code for content-summary implementations, commands, helpers, and algorithm-shaped validators; remove every executable occurrence.
- Search current contracts for removed integrity fields and remove them. Keep any required literal legacy key only in an explicit compatibility fixture.
- Verify no replacement content-derived identity or integrity algorithm was introduced.
