...
- Create `pipeline-core/src/main/kotlin/rs/masumi/core/ocr/OcrIdentity.kt`: deterministic OCR region, page, and run keys.
...
- Create `pipeline-core/src/main/kotlin/rs/masumi/core/modelpackage/OcrModelPackageStore.kt`: resumable, hash-verified two-file publication.
...
`OcrIdentity` canonical-hashes ordered name/length/value fields and exposes:

```kotlin
fun regionId(pageId: String, detectionPageArtifactKey: String, sourceRegionIds: List<String>, semantic: OcrSemanticStatus): String
fun pageArtifactKey(sourceSha256: String, detectionPageArtifactKey: String, dependencies: OcrDependencies): String
fun runArtifactKey(entries: List<Pair<Int, String>>): String
```

Include every config/model/runtime/prompt dependency in `pageArtifactKey`; exclude output, timing, project, job, filename, and timestamp fields.
...
Use generated byte arrays and fake ranged responses. Cover resume from a valid partial, restart on `200`, rejection and one clean retry on bad length/hash, both-file capability validation, and no publication after the first file only.
...
Pin revision `511b09642bb324401f15f97cc23bc67e8f0a291d`, model length/hash `935769056`/`f3ae46ec885050acf4b3d31944431e1fd90d50664fb09126af4a3c050ba14ee8`, projector length/hash `881770560`/`204d757d7610d9b3faab10d506d69e5b244e32bf765e2bab2d0167e65e0a058a`, Apache-2.0, prompt `OCR:`, llama tag `b8935`, and runtime `arm64-v8a:cpu:mtmd`. The store must never treat one installed file or a `*.part` as a complete package.
...
Document that OCR is implemented, model download size is approximately 1.8 GB, local arm64 inference is sequential, uncertainty is protected, and translation/cleanup/typesetting/export remain deferred. Do not include corpus, device, account, path, hash, screenshot, timing sample, or private output details.
