...
class OnnxAotInpainterTest {
    ...
    @Test
    fun engineUsesNeuralPathForFreeTextAndFallsBackWhenItFails() {
        loadInpainter().use { inpainter ->
            var neuralCalls = 0
            val counting = NeuralInpainter { pixels, w, h, l, t, r, b, m ->
                neuralCalls += 1
                inpainter.inpaint(pixels, w, h, l, t, r, b, m)
            }
            ...
            val cleaned = SourceCleanupEngine(counting).clean(
                source,
                listOf(
                    CleanupTarget(
            ...
            // A failing neural inpainter must fall back to interpolation and
            // still clean the region.
            val failing = NeuralInpainter { _, _, _, _, _, _, _, _ -> false }
            val fallbackSource = Bitmap.createBitmap(160, 128, Bitmap.Config.ARGB_8888).apply {
                eraseColor(Color.WHITE)
                for (y in 52 until 76) for (x in 66 until 94) setPixel(x, y, Color.BLACK)
            }
            val fallback = SourceCleanupEngine(failing).clean(
                fallbackSource,
                listOf(
                    CleanupTarget(
                        translationRegionId = "a".repeat(64),
            ...
            try {
                assertEquals(CleanupRegionState.CLEANED, fallback.regions.single().state)
                assertTrue(Color.red(fallback.bitmap.getPixel(80, 64)) > 100)
            } finally {
                fallback.bitmap.recycle()
                fallbackSource.recycle()
            }
        }
    }
}
