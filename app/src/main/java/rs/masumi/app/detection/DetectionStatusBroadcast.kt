...
object DetectionStatusBroadcast {
    ...
    const val EXTRA_PRESERVED_PAGE_COUNT = "preserved_page_count"
    ...
    fun create(packageName: String, progress: DetectionProgress): Intent {
        ...
        return Intent(ACTION).apply {
            ...
            putExtra(EXTRA_PRESERVED_PAGE_COUNT, progress.preservedPageCount)
    ...
    fun parse(intent: Intent): DetectionProgress? = runCatching {
        ...
        val runArtifactKey = requireNotNull(intent.getStringExtra(EXTRA_RUN_ARTIFACT_KEY))
        ...
        DetectionProgress(
            ...
            preservedPageCount = intent.requireNonNegativeInt(EXTRA_PRESERVED_PAGE_COUNT),
            ...
            totalBytes = intent.requireNonNegativeLong(EXTRA_TOTAL_BYTES),
            errorCode = errorCode,
        )
    }.getOrNull()
    ...
    private fun Intent.requireNonNegativeLong(key: String): Long =
        getLongExtra(key, INVALID_LONG).also { require(it >= 0) }
