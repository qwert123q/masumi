...
class LibraryActivity : Activity() {
    ...
    private fun buildSummary(
        ...
        val translation = ocr?.let { catalog.latestPublishedTranslationRun(projectId) }
            ?.takeIf { it.artifact.dependencies.ocrRunArtifactKey == ocr.artifact.runArtifactKey }
        ...
        val issuePageCount = quality?.report?.let { it.warningPageCount + it.blockedPageCount } ?: 0
        val cover = store.outputPages(projectId).firstOrNull()?.uri
    ...
    private fun createProjectItem(rootUri: Uri?, summary: LibraryProjectSummary): View {
        ...
        summary.coverUri?.let { uri ->
            decodeCover(uri)?.let { bitmap ->
        ...
        if (summary.issuePageCount > 0) {
            secondary.text = getString(R.string.library_review_issues, summary.issuePageCount)
            secondary.setOnClickListener {
                startActivity(IssueReviewActivity.intent(this, project.metadata.projectId, project.metadata.title))
            }
        } else {
    ...
    private fun statusText(summary: LibraryProjectSummary): String = when {
        ...
        summary.project.outputPageCount > 0 && summary.issuePageCount > 0 ->
            getString(R.string.library_project_status_ready_issues, summary.project.outputPageCount, summary.issuePageCount)
        summary.project.outputPageCount > 0 ->
            getString(R.string.library_project_status_ready, summary.project.outputPageCount)
        summary.issuePageCount > 0 ->
            getString(R.string.library_project_status_needs_review, summary.issuePageCount)
        else ->
            getString(
    ...
    private fun decodeCover(uri: Uri): Bitmap? {
